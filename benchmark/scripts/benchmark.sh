...
#   --save           自动保存结果
#   --no-save        预览模式（不保存结果）
#   --scheduler      运行调度器触发压测（含 mock webhook）
#   --workload <名称>  负载分布: uniform, prod-typical, burst-ultra, mixed-heavy
#   --json           同时输出 JSON 格式
...
for arg in "$@"; do
    case $arg in
        ...
        *)
            echo "未知选项: $arg"
            echo "用法: $0 [--internal] [--http] [--scheduler] [--quick] [--compare] [--save] [--no-save] [--workload <name>] [--json]"
            exit 1
            ;;
    esac
done
...
# 编译项目
echo ">>> 编译项目..."
mvn compile test-compile -q -DskipTests 2>&1 | tail -3
...
if [ "$TEST_MODE" = "internal" ]; then
    ...
    BENCHMARK_CLASS="com.loomq.scheduler.SchedulerTriggerBenchmarkWithMockServer"
    mvn exec:java \
        -Dexec.mainClass="$BENCHMARK_CLASS" \
        -Dexec.classpathScope=test \
        -Dloomq.benchmark.workload="$WORKLOAD" \
        -Dloomq.benchmark.quick="$QUICK_MODE" \
        -q
...
echo "      使用 --scheduler 运行调度器压测"
echo "      使用 --workload prod-typical 测试生产负载分布"
