...
if (-not $SkipBenchmark) {
    ...
    foreach ($line in $r.Output) {
        if ($line -match '^(RESULT_[A-Z0-9_]+)\|(.+)') {
            ...
            # Final summary marker (RESULT|peak_tier=...|completion_rate=...)
            foreach ($pair in $Matches[1].Split('|')) {
                $kv = $pair.Split('=', 2)
                if ($kv.Count -eq 2) { $bmResult[$kv[0]] = $kv[1] }
            }
        }
    }
...
foreach ($t in $tiers) {
    $cols += "${t}_qps", "${t}_p95_ms", "${t}_p99_ms",
             ...
             "${t}_util_pct", "${t}_backpressure"
}
$cols += "completion_rate","total_qps","global_p95_total_ms","vt_reduction_pct","cohort_wake_events"
$ExpectedHeader = $cols -join ","

# Header consistency: verify or rebuild
$NeedsNewHeader = $true
if (Test-Path $CsvFile) {
    $FirstLine = (Get-Content $CsvFile -TotalCount 1 -ErrorAction SilentlyContinue)
    if ($FirstLine -eq $ExpectedHeader) {
        $NeedsNewHeader = $false
    }
}
if ($NeedsNewHeader) {
    [System.IO.File]::WriteAllLines($CsvFile, @($ExpectedHeader), [System.Text.UTF8Encoding]::new($false))
}
...
$envData = if ($bmData) { $bmData["env"] } else { $null }
...
$sysQps = if ($bmData -and $bmData["system_qps"]) { $bmData["system_qps"] } else { $null }

$vals += if ($bmResult -and $bmResult.Count -gt 0) { $bmResult["completion_rate"] } else { "" }
$vals += if ($sysQps -and $sysQps.Count -gt 0) { $sysQps["total_qps"] } else { "" }
$vals += if ($global) { $global["p95_total"] } else { "" }
$vals += if ($opt) { $opt["vt_reduction_pct"] } else { "" }
$vals += if ($cohort) { $cohort["wake_events"] } else { "" }

$RowLine = $vals -join ","
[System.IO.File]::AppendAllText($CsvFile, $RowLine + [Environment]::NewLine, [System.Text.UTF8Encoding]::new($false))
