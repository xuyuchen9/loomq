...
if (-not $NoSave) {
    ...
    if ($bmData -and $bmData["rows"] -and $bmData["rows"].Count -gt 0) {
        ...
        foreach ($t in $tiers) {
            ...
            $limits = $slo[$t]
            if ($lat -and $limits) {
                $p95ok = [int]$lat["p95"] -le $limits["p95"]
                $p99ok = [int]$lat["p99"] -le $limits["p99"]
                $lines += ("{0,-10} {1,13} {2,13} {3,8} {4,13} {5,13} {6,8}" -f
                    $t, "$($lat['p95'])ms", "$($limits['p95'])ms", $(if ($p95ok) { "PASS" } else { "FAIL" }),
                    "$($lat['p99'])ms", "$($limits['p99'])ms", $(if ($p99ok) { "PASS" } else { "FAIL" }))
            }
        }
        $lines += ""
