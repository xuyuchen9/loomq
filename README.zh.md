...
| **Beta** | Raft 领导选举、日志复制、快照追赶、leader-authoritative 读、Raft 可观测性 |
| **尚未承诺** | 分布式协调原语、锁/租约语义 |
...
- `GET /v1/intents/{intentId}` 变成 leader-authoritative
- follower 会返回 `503`，错误码 `50301`，`retryable=true`，并在已知时带上 leader id
- `/health` 和 `/metrics` 会暴露 Raft 信号，例如角色、leader id、term、commit index、commit lag、replication lag、peer 可达性，以及 leader 当前是否还能安全接收读 / 写
- `/health/deep` 会在 Raft 视图之上继续附加各精度档位的 backpressure 数据
